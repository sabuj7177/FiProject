...
import TensorFI.faultTypes as ft
...
def main():
    ...
    if(isTest):
        ...
        print_labels_from_index(np.argmax(ty[:20, :], axis=1))
        preds = sess.run(y_pred_cls, feed_dict={x: tx, y: ty})
        print("without fault injection")
        print_labels_from_index(preds[:20])
        correct = (np.argmax(ty, axis=1) == preds)
        correctIndex = np.argwhere(correct == True)
        correctIndex = correctIndex.flatten()
        print("Total correct is " + str(len(correctIndex)) + " among " + str(len(ty)))
        ...
        preds2 = sess.run(y_pred_cls, feed_dict={x: tx, y: ty})
        print("with fault injection")
        print_labels_from_index(preds2[:20])
        correct2 = (np.argmax(ty, axis=1) == preds2)
        correctIndex2 = np.argwhere(correct2 == True)
        correctIndex2 = correctIndex2.flatten()
        print("Total correct is " + str(len(correctIndex2)) + " among " + str(len(ty)))
